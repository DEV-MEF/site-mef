...
const AllFiles = ({ params }: { params: Promise<{ documentId: string }> }) => {
  ...
  const { folders, foldersSelected, setFoldersSelected } = useHookFolders(
    "document",
    documentId
  );
    ...
    return (
      <section className="w-full">
        <Banner
          ...
          text_3={
            foldersSelected[foldersSelected.length - 1]?.name || documentId
          }
        />
      ...
      <div className="w-full container px-4 max-w-[88rem] mx-auto py-10">
        ...
        <div className="w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mt-14">
          {files.map((doc, index) => (
            <div
              key={index}
              className="flex flex-col lg:flex-row items-start lg:items-center justify-between bg-white border border-[#E2E8F0] rounded-lg p-[14px] efects hover:border-[#5151F8]"
            >
              {/* Ícone do documento */}
